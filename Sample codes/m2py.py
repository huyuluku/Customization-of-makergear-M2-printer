...
class Makergear:
    ...
    def close(self, zrange = [0, 203]):
        """
        Closes the specified handle. If self.printout = 1, this function will close the necessary serial object.
        If prinout = 0, this function will close the specified temporary file and plot a visualization of all relevant movement commands.
        Visualization function will use whatever coordinate system you explicity designate using coord.
        If coord isn't explicitly called, the coordinate system used by the visualization tool will be absolute.
        """
        if self.printout == 1:
            self.alloff()
            self.rotate(speed = 0)
    ...
    def set_bed_temp(self, temp = 25, wait = 'off'):
        """
        Sets the temperature of the heated bed to the specified temp. If the wait argument is set to 'on',
        the printer will wait for temp to be reached before excecuting other commands.
        If 'off' the printer will set the temp without waiting.
        """
        if temp > 100: 
            self.close()
